...
if __name__ == '__main__':
    ...
    if is_main_process():
        metric_guide_markdown = """
            ## Metric Guide & Expected Behavior

            This guide helps interpret the training charts logged during the session.
            ...
            * **`train/loss`**: The loss value calculated on the current training batch.
                ...
                * **Warning:** * Flat line from start: Model is not learning (LR too low or bug).
            ...
            * **`eval/{name}/loss_quantile_X.X`**: Validation loss for specific noise levels (timesteps).
            ...
            ### 2. Stability Metrics
            * **`train/grad_norm`**: The L2 norm of the gradients (force of the update) before clipping.
                * **Expected:** Stable values after warmup. It is clipped to `gradient_clipping` (default 1.0), so printed values usually cap there, but raw values tell you the signal strength.
                * **Warning:** Exploding values (huge spikes) mean instability. Near-zero values mean vanishing gradients (model stopped learning).
        ...
        # Log this text to the "TEXT" tab in TensorBoard at step 0
        tb_writer.add_text('Metric Guide', metric_guide_markdown, 0)
